...
contract ExponentialPricing {
    ...
    uint256 private constant E = 2718281828459044864;
    ...
    function getTvMidPrice(int256 timePrice, int256 deltaB, int256 maxB, int256 K) external pure returns (int256) {
        if (maxB <=0) {
            return timePrice;
        }
        int256 midPrice = timePrice * exp(E, (K * deltaB / maxB)) / ONE;
        return midPrice;
    }

    function queryTradePMM(int256 timePrice, int256 deltaB, int256 volume, int256 maxB, int256 K) external pure returns (int256) {
        int256 r = maxB * (exp(E, (K * (volume + deltaB) / maxB)) - exp(E, (K * deltaB / maxB))) / K;
