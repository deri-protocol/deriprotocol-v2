...
import "../interface/IEverlastingOption1.sol";
import '../library/SafeMath.sol';
import '../library/SafeERC20.sol';
import '../utils/Migratable.sol';

contract EverlastingOption1 is IEverlastingOption1, Migratable {
    ...
    function executeMigration(address source) external override _controller_ {
        uint256 migrationTimestamp_ = IEverlastingOption1(source).migrationTimestamp();
        address migrationDestination_ = IEverlastingOption1(source).migrationDestination();
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            _symbols[symbolIds[i]] = IEverlastingOption1(source).getSymbol(symbolIds[i]);
        }

        // transfer state values
        (_liquidity, _lastTimestamp, _protocolFeeAccrued) = IEverlastingOption1(source).getPoolStateValues();
